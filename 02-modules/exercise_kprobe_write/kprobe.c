...
static unsigned int tracked_fd = 10; // Default channel
...
static int param_set_tracked_fd(const char *val, const struct kernel_param *kp)
{
    ...
    err = kstrtouint(val, 10, &new_fd);
    if (err) {
	pr_warn("Invalid channel set via module parameter (must be positive)\n");
        return -EINVAL;
    }

    if (new_fd != tracked_fd) {
	pr_info("%zu bytes written at channel %u\n", bytes_written, tracked_fd);
        pr_info("tracked_fd modified dynamically: %u -> %u\n", tracked_fd, new_fd);
	bytes_written = 0;
...
static const struct kernel_param_ops tracked_fd_ops = {
    ...
    .get = param_get_uint,       // Default get function (cat /sys/module/...)
};
...
{
    /* ksys_write(unsigned int fd, const char __user *addr, size_t count)
     * Arg 0: fd
     * Arg 2: count 
     */
    unsigned int fd = regs_get_kernel_argument(regs, 0);

    if (fd == READ_ONCE(tracked_fd)) { 
    	size_t count = regs_get_kernel_argument(regs, 2);
	bytes_written += count;
	if (count == 0)
	 pr_info("Current state: %zu bytes written so far at channel %u\n", bytes_written, tracked_fd);
    }
    return 0;
}
...
static int __init kprobe_init(void)
{
    ...
    pr_info("module loaded (monitoring writes at channel %u)\n", tracked_fd);
    return 0;
}
...
static void __exit kprobe_exit(void)
{
    ...
    pr_info("module unloaded: %zu bytes written at channel %u\n", bytes_written, tracked_fd);
}
